...
class FeatureExtractor(object):
    ...
    def extract_features(self):
        ...
        for i_split in range(self.n_splits):
            ...
            models = [model] if hasattr(model, "predict") else model
            for index, clf in enumerate(models):
                format_plot()
                # fig, [ax_roc_train, ax_roc, ax_conf] = plt.subplots(1, 3, figsize=(12, 5))
                fig, [ax_roc_train, ax_roc] = plt.subplots(1, 2, figsize=(12, 5))
                fig.tight_layout()

                if self.name == "MLP":
                    tmp_labels = np.zeros((train_set.shape[0], 2))
                    tmp_labels[train_labels[:, index] == 1, 0] = 1
                    tmp_labels[train_labels[:, index] != 1, 1] = 1
                    tmp_labels = tmp_labels[:, 1]
                else:
                    tmp_labels = np.zeros(train_set.shape[0])
                    tmp_labels[train_labels[:, index] == 1] = 1

                RocCurveDisplay.from_estimator(clf, train_set, tmp_labels, ax=ax_roc_train)
                
                if self.name == "MLP":
                    tmp_labels = np.zeros((test_set.shape[0], 2))
                    tmp_labels[test_labels[:, index] == 1, 0] = 1
                    tmp_labels[test_labels[:, index] != 1, 1] = 1
                    tmp_labels = tmp_labels[:, 1]

                    predictions = clf.predict(test_set)[:, 1]
                    # cm = confusion_matrix(tmp_labels, predictions)
                    # disp = ConfusionMatrixDisplay(confusion_matrix=cm)
                    # disp.plot(ax=ax_conf)
                else:
                    tmp_labels = np.zeros(test_set.shape[0])
                    tmp_labels[test_labels[:, index] == 1] = 1
                    predictions = clf.predict(test_set)
                    # cm = confusion_matrix(tmp_labels, predictions)
                    # disp = ConfusionMatrixDisplay(confusion_matrix=cm)
                    # disp.plot(ax=ax_conf)

                RocCurveDisplay.from_estimator(clf, test_set, tmp_labels, ax=ax_roc)

                ax_roc_train.set_title(f'{self.name} Train ROC', fontweight='bold')
                ax_roc.set_title(f'{self.name} ROC', fontweight='bold')
                # ax_conf.set_title(f'{self.name} Confusion Matrix')

                ext = "svg"
                plt.savefig(f'{self.name}_{index + 1}.{ext}', bbox_inches="tight", format=ext)
                plt.clf() 
