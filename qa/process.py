...
def script_grabber():
    ...
    for lines in range(0,len(fileinfosort)):
        ...
        if lines < len(fileinfosort)-1:
         if lines != 0:
          for line2 in range(1,fileinfosort[lines+1][0]-fileinfosort[lines][0]+1):
              allcharge.write(chargefile[line2])
          for line3 in range(0,(fileinfosort[lines+1][0]-fileinfosort[lines][0])*nat):
              allcoors.write(coorsfile[line3])
        if lines == len(fileinfosort)-1:
...
def charge_grab():
           ...
           if pdbfile[lines].split()[5] != oldresi:
              ...
              if pdbfile[lines].split()[2] != 'N':
                if pdbfile[lines].split()[2] != 'O':
                   if pdbfile[lines].split()[2] != 'C':
                     if pdbfile[lines].split()[2] != 'H':
                           atwsclist[int(oldresi)-1].append(pdbfile[lines].split()[1])
    #Note 10/15 - working on this for sidechains - think I got it working... 
    bbchargearray=[]
    scchargearray=[]
    ...
    for line2 in range(1,len(chargefile)):
         ...
         for resat in range(0,len(atwbblist)):
             ...
             for atwbbs in range(0,len(atwbblist[resat])):
                 rescharge+=float(chargefile[line2].split()[int(atwbblist[resat][atwbbs])-1])
             bbchargearray[line2-1].append(rescharge)
         for resat in range(0,len(atwsclist)):
             ressccharge=0.00
             for atwscs in range(0,len(atwsclist[resat])):
                 ressccharge+=float(chargefile[line2].split()[int(atwsclist[resat][atwscs])-1])
    ...
    for resi in range(0,len(reslist)):
       print('%s %f %f %f %f %f' %(reslist[resi],maxchgbb[resi],minchgbb[resi],maxchgbb[resi]-minchgbb[resi],avgchgbb[resi],stdchgbb[resi]))
    ...
    for resi in range(0,len(reslist)):
       print('%s %f %f %f %f %f' %(reslist[resi],maxchgsc[resi],minchgsc[resi],maxchgsc[resi]-minchgsc[resi],avgchgsc[resi],stdchgsc[resi]))
